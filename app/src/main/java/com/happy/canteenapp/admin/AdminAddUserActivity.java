...
import com.happy.canteenapp.MainActivity;
import com.happy.canteenapp.R;
import com.happy.canteenapp.User;

public class AdminAddUserActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener
{
    ...
    String[] usertypes = {"User", "Admin", "Student", "Teacher","Canteen"};
    String usertype;
                ...
                auth.createUserWithEmailAndPassword(email, password)
                        .addOnCompleteListener(AdminAddUserActivity.this, new OnCompleteListener<AuthResult>() {
                            ...
                            public void onComplete(@NonNull Task<AuthResult> task) {
                                Toast.makeText(AdminAddUserActivity.this, "createUserWithEmail:onComplete:" + task.isSuccessful(), Toast.LENGTH_SHORT).show();
                                progressBar.setVisibility(View.GONE);
                                // If sign in fails, display a message to the user. If sign in succeeds
                                // the auth state listener will be notified and logic to handle the
                                // signed in user can be handled in the listener.
                                if (!task.isSuccessful()) {
                                    Toast.makeText(AdminAddUserActivity.this, "Authentication failed." + task.getException(),
                                            Toast.LENGTH_SHORT).show();
                                } else {
                                    generateUser(username, password, usertype);
                                    startActivity(new Intent(AdminAddUserActivity.this, MainActivity.class));
                                    finish();
                                }
                            }
                        });
